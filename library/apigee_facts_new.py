...
def get_apigee_facts(hostvars, topology, my_hostname):
    ...
    if me in ldap_hosts:
        if not ldap_replication:
            ...
            try:
                ...
                facts['ldap_sid'] = 1
            ldap_primary = ldap_only_hosts[0]
    ...
    if 'dp' in facts['profiles']:
        if 'pdb' in facts['profiles']:
            ...
            local_dp_pgsqls = planet.get_services('DevPortalPostgreSQL')
            local_pgsqls = planet.get_services('PostgreSQL')
            if local_dp_pgsqls:
